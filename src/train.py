...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % VAL_EVERY == 0:
            val_psnr, val_ssim = evaluate_loader(model, test_loader, device)
            ...
            if val_psnr > best_psnr:
                best_psnr = val_psnr
                best_path = os.path.join(CHECKPOINT_DIR, "best_lol_unet.pt")
                torch.save({
                    "epoch": epoch,
        ...
        if epoch % SAVE_EVERY == 0:
            ckpt_path = os.path.join(CHECKPOINT_DIR, f"lol_unet_ep{epoch}.pt")
            torch.save({
                "epoch": epoch,
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
